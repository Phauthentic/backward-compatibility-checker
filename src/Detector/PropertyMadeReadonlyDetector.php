...
final readonly class PropertyMadeReadonlyDetector implements BcBreakDetectorInterface
{
    public function detect(ClassInfo $before, ClassInfo $after): array
    {
        ...
        foreach ($before->getPublicOrProtectedProperties() as $beforeProperty) {
            ...
            if (!$beforeProperty->isReadonly && $afterProperty->isReadonly) {
                $breaks[] = new BcBreak(
                    message: sprintf(
                        'Property %s::$%s was made readonly',
                        $before->name,
                        $beforeProperty->name,
                    ),
                    className: $before->name,
                    memberName: $beforeProperty->name,
                    type: BcBreakType::PropertyMadeReadonly,
                );
            }
        }

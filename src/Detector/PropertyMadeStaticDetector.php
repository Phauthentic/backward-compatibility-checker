...
final readonly class PropertyMadeStaticDetector implements BcBreakDetectorInterface
{
    public function detect(ClassInfo $before, ClassInfo $after): array
    {
        ...
        foreach ($before->getPublicOrProtectedProperties() as $beforeProperty) {
            ...
            if (!$beforeProperty->isStatic && $afterProperty->isStatic) {
                $breaks[] = new BcBreak(
                    message: sprintf(
                        'Property %s::$%s was made static',
                        $before->name,
                        $beforeProperty->name,
                    ),
                    className: $before->name,
                    memberName: $beforeProperty->name,
                    type: BcBreakType::PropertyMadeStatic,
                );
            }
            ...
            if ($beforeProperty->isStatic && !$afterProperty->isStatic) {
                $breaks[] = new BcBreak(
                    message: sprintf(
                        'Property %s::$%s is no longer static',
                        $before->name,
                        $beforeProperty->name,
                    ),
                    className: $before->name,
                    memberName: $beforeProperty->name,
                    type: BcBreakType::PropertyMadeNonStatic,
                );
            }
        }

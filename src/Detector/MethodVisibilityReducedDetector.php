...
final readonly class MethodVisibilityReducedDetector implements BcBreakDetectorInterface
{
    public function detect(ClassInfo $before, ClassInfo $after): array
    {
        ...
        foreach ($before->getPublicOrProtectedMethods() as $beforeMethod) {
            ...
            if ($afterMethod->visibility->isMoreRestrictiveThan($beforeMethod->visibility)) {
                $breaks[] = new BcBreak(
                    message: sprintf(
                        'Method %s::%s() visibility reduced from %s to %s',
                        $before->name,
                        $beforeMethod->name,
                        $beforeMethod->visibility->value,
                        $afterMethod->visibility->value,
                    ),
                    className: $before->name,
                    memberName: $beforeMethod->name,
                    type: BcBreakType::MethodVisibilityReduced,
                );
            }
        }

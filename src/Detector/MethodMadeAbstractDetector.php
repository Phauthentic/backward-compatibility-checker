...
final readonly class MethodMadeAbstractDetector implements BcBreakDetectorInterface
{
    public function detect(ClassInfo $before, ClassInfo $after): array
    {
        ...
        foreach ($before->getPublicOrProtectedMethods() as $beforeMethod) {
            ...
            if (!$beforeMethod->isAbstract && $afterMethod->isAbstract) {
                $breaks[] = new BcBreak(
                    message: sprintf(
                        'Method %s::%s() was made abstract',
                        $before->name,
                        $beforeMethod->name,
                    ),
                    className: $before->name,
                    memberName: $beforeMethod->name,
                    type: BcBreakType::MethodMadeAbstract,
                );
            }
        }

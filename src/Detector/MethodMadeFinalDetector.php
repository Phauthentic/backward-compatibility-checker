...
final readonly class MethodMadeFinalDetector implements BcBreakDetectorInterface
{
    public function detect(ClassInfo $before, ClassInfo $after): array
    {
        ...
        foreach ($before->getPublicOrProtectedMethods() as $beforeMethod) {
            ...
            if (!$beforeMethod->isFinal && $afterMethod->isFinal) {
                $breaks[] = new BcBreak(
                    message: sprintf(
                        'Method %s::%s() was made final',
                        $before->name,
                        $beforeMethod->name,
                    ),
                    className: $before->name,
                    memberName: $beforeMethod->name,
                    type: BcBreakType::MethodMadeFinal,
                );
            }
        }

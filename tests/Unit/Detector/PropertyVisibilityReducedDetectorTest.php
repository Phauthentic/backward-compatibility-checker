...
#[CoversClass(PropertyVisibilityReducedDetector::class)]
final class PropertyVisibilityReducedDetectorTest extends TestCase
{
    private PropertyVisibilityReducedDetector $detector;

    protected function setUp(): void
    {
        $this->detector = new PropertyVisibilityReducedDetector();
    }
    ...
    public function testDetectsPublicToProtected(): void
    {
        $before = new ClassInfo(
            name: 'App\\Entity',
            properties: [new PropertyInfo(name: 'name', visibility: Visibility::Public)],
        );
        $after = new ClassInfo(
            name: 'App\\Entity',
            properties: [new PropertyInfo(name: 'name', visibility: Visibility::Protected)],
        );

        $breaks = $this->detector->detect($before, $after);

        $this->assertCount(1, $breaks);
        $this->assertSame(BcBreakType::PropertyVisibilityReduced, $breaks[0]->type);
    ...
    public function testDetectsPublicToPrivate(): void
    {
        ...
        $after = new ClassInfo(
            name: 'App\\Entity',
            properties: [new PropertyInfo(name: 'name', visibility: Visibility::Private)],
        );
